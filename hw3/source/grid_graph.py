...
class grid_graph:
    def __init__(self, grid_map_matrix=None, xy_reso=0.1*np.ones((2, 1))):
        self.grid_map = grid_map_matrix
        self.node_tuple = namedtuple('graph_node', 'x, y, cost, parent')
    ...
    def pose_to_index(self, pos_x, pos_y):
        ...
        index_x = int(pos_x / self.xy_reso[0, 0])
        index_y = int(pos_y / self.xy_reso[1, 0])

        return index_x, index_y

    def index_to_pose(self, index_x, index_y):
        
        pos_x = index_x * self.xy_reso[0, 0]
        pos_y = index_y * self.xy_reso[1, 0]
